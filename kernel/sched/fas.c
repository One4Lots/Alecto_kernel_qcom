...
static unsigned int fas_boost_ms = 30;
...
static void fas_do_boost(struct work_struct *work)
{
	...
	for_each_possible_cpu(i) {
		s = &per_cpu(fas_sync_info, i);
		if (fps <= 30) {
			s->boost_min = (i <= 5) ? 768000 :   /* Little cluster (CPUs 0-5) */
				       (i <= 7) ? 825600 : 0;  /* Big cluster (CPUs 6-7) */
		} else {
			s->boost_min = (i <= 5) ? 1324800 :  /* Little cluster (CPUs 0-5) */
				       (i <= 7) ? 1708800 : 0; /* Big cluster (CPUs 6-7) */
		}
	}
